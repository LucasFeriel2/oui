...
function Profil() {
    ...
    const deleteUser = async () =>{
      ...
      const updateUser = async () =>{
        try {
          axios.post('http://localhost:3000/api/delete', {displayName: user, email: email, password: password})
        .then(res => {
          console.log(res);
          console.log(res.data);
        })
  ...
  return (
    <div className="App">
      ...
      <h1 className='absolute w-11/12 h-92 top-32 font-normal text-6xl leading-10 text-center' style={{color: "#FFFFFF"}}>Modifier votre profil</h1>
      <div className='absolute left-1/4 top-1/4 w-1/2 h-1/2'>
        <form onSubmit={updateUser}>
            <label className='absolute w-80 h-8 left-1/3 top-4 '>Email</label>
            <input className='absolute w-96 h-20 pl-12 left-72 top-12'  onChange={e => setEmail(e.target.value)} name="email" type="text" style={{background: "#FFFFFF", borderRadius: "20px"}}></input>
            ...
            <input className='absolute w-96 h-20 pl-12 left-72 top-48'  onChange={e => setUser(e.target.value)} name="user" type="text" style={{background: "#FFFFFF", borderRadius: "20px"}}></input>          
            ...
            <input className='absolute w-96 h-20 pl-12 left-72 bottom-20'  onChange={e => setPassword(e.target.value)} name="mdp" type="password" style={{background: "#FFFFFF", borderRadius: "20px"}}></input>
          ...
          <button className='absolute w-96 h-20 left-72 -bottom-20' type="submit" style={{background: "#393939", borderRadius: "20px"}}>Modifier</button>
          <button className='absolute w-96 h-20 left-72 -bottom-44' style={{background: "#FF0000", borderRadius: "20px"}} onClick={deleteUser}>Supprimer compte</button>
        </form>
      </div>
    </div>
  );
